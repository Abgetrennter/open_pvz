...
| `MechanicFamilyRegistry.gd` | Node | 10 个冻结 Mechanic family 注册（冻结，新增需 ADR） |
...
- `scripts/core/registry/registry_base.gd` -- 10 个 Registry autoload 的基类，提供注册、去重、信任检查、来源追踪
- `scripts/core/runtime/protocol_validator.gd` -- 所有注册表用于定义验证（参数类型、边界、资源脚本类型）
- `scripts/core/defs/` -- TriggerDef / EffectDef / DetectionDef / ControllerDef / ProjectileMovementDef / MechanicCompilerDef 等资源定义
