...
Open PVZ 的规则引擎骨架（事件 → 触发 → 效果 → 执行链）已成熟，战斗子系统（经济、棋盘、卡片、波次、流程）均有状态类实现。当前第一批 UI / 输入原型已经迁入 `scripts/ui/` 与 `scripts/input/`，并在 demo 场景与输入验证场景中替换旧实现；`scripts/demo/` 只保留场景编排器和仍被复用的格子可视节点。与此同时，扩展系统总体规划（[38-扩展系统总体规划](../wiki/04-roadmap-reference/38-扩展系统总体规划.md)）已将引擎定位为平台层，定义了 6 类扩展包（内容包、素材包、规则包、集合包、本地化包、UI 主题包）。
...
UI 框架层需要对接扩展系统总体规划（[38-扩展系统总体规划](../wiki/04-roadmap-reference/38-扩展系统总体规划.md)）中定义的 4 类扩展包：
...
本节描述 UI 框架层与扩展系统总体规划（[38-扩展系统总体规划](../wiki/04-roadmap-reference/38-扩展系统总体规划.md)）的集成路线。
