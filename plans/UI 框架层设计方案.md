# UI 框架层设计方案

> **文档性质**：设计文档（Design Document）
> **所属圈层**：表现、验证与治理层
> **关联系统**：战斗玩法层、输入交互层
> **状态**：待审批

---

## Context

Open PVZ 的规则引擎骨架（事件 → 触发 → 效果 → 执行链）已成熟，战斗子系统（经济、棋盘、卡片、波次、流程）均有状态类实现。当前第一批 UI / 输入原型已经迁入 `scripts/ui/` 与 `scripts/input/`，并在 demo 场景与输入验证场景中替换旧实现；`scripts/demo/` 只保留场景编排器和仍被复用的格子可视节点。与此同时，扩展系统总体规划（[38-扩展系统总体规划-v1](../wiki/04-roadmap-reference/38-扩展系统总体规划-v1.md)）已将引擎定位为平台层，定义了 6 类扩展包（内容包、素材包、规则包、集合包、本地化包、UI 主题包）。

本方案的目标：**将 demo 原型提升为可组合、可测试、与引擎解耦的 UI 框架层**，作为当前阶段的**支撑层规范化方案**，为后续内容开发和表现层扩展奠定基础。同时，**预留扩展系统对接接口**，确保 UI 面板能被 `ui_theme_pack` 驱动样式、通过 `AssetRegistry` 解析素材、响应 `content_pack` 的动态注册，而无需后续重构。

**不包含**：具体视觉美化、素材集成、动画资源、音频 UI 反馈、扩展系统本身的实现。

---

## 1. 架构定位

### 1.1 在五层模型中的位置

```
引擎核（已完成）→ 战斗玩法层（已完成）→ 内容表达层（进行中）
                                          ↑
                              UI 框架层（本方案）  ← 表现、验证与治理层
                                          ↓
                              输入交互层（InputRouter，已有设计）
                                          ↑
                              扩展平台层（规划中）— ui_theme_pack / asset_pack / content_pack / localization_pack
```

UI 框架层位于**战斗玩法层之上、输入交互层之下**，同时**向上对接扩展平台层**。它：
- **观察**战斗状态（通过 EventBus 订阅）
- **呈现**战斗信息（通过 Control 节点树）
- **转发**用户操作（通过信号 → InputRouter）
- **不驱动**任何规则逻辑
- **预留**扩展系统接口（主题注入、素材解析、动态内容、本地化），当前以占位实现，待扩展平台层落地后切换

### 1.2 核心原则

| 原则 | 含义 |
|------|------|
| 单向数据流 | 战斗状态 → EventBus → UI 面板 → 叶子控件。UI 永远不直接修改战斗状态 |
| UI 可选 | BattleManager 不依赖任何 UI。验证场景不创建 UI 也能正常运行 |
| 中介者模式 | 每个面板（CardBar、SunCounter）充当中介者，订阅 EventBus 并驱动子控件。叶子控件不订阅 EventBus |
| 无 UIModel 层 | 战斗子系统即 Model，EventBus 即观察通道。不引入额外的状态缓存层 |
| 资源驱动 | 面板布局参数、交互配置通过 `.tres` Resource 定义 |
| 扩展接口预留 | 面板不硬编码视觉素材路径，使用逻辑 ID（`icon_id`、`theme_id`）；待 `AssetRegistry` / `LocalizationRegistry` 落地后切换。当前以占位实现 |
| 主题可切换 | 面板样式（颜色、字体、布局参数）通过 `UIThemeProfile` Resource 注入，支持 `ui_theme_pack` 替换 |

### 1.3 关键架构决策

**ADR-UI-001：中介者而非直接订阅**
- 每个逻辑面板（如 CardBar）订阅 EventBus，再驱动内部子控件
- 50+ 叶子控件直接订阅 EventBus 会导致事件流不可追踪
- 中介者可批量更新、防抖、过滤

**ADR-UI-002：不设 UIModel 层**
- BattleEconomyState、BattleCardState 等已是 Model
- 面板内部可缓存渲染所需值（如 `_current_sun`），但这不是独立 Model 层

**ADR-UI-003：UI 通过 overlay 模式接入**
- BattleManager 不引用任何 UI 类型
- UI 由场景编排层（DemoBattleScene 或未来的 BattleScene）创建和接线
- 无 UI 时 BattleManager 完全可用

**ADR-UI-004：主题通过 Resource 注入，不硬编码**
- 面板样式参数（颜色、字体、间距等）通过 `UIThemeProfile` Resource 注入
- 主仓内置默认 `UIThemeProfile`，`ui_theme_pack` 可提供替代配置
- 面板代码不直接写死 `Color(1, 1, 0)` 等视觉常量
- 当前阶段：`UIThemeProfile` 仅定义签名，面板仍使用内联默认值，但所有视觉常量统一收敛到 `_get_theme()` 方法

**ADR-UI-005：素材使用逻辑 ID，延迟绑定真实资源**
- 面板中需要图标/贴图/音效的地方，使用逻辑 ID（如 `icon_id = "sun"`）而非文件路径
- 当前阶段：`AssetRegistry` 尚未落地，面板通过 `_resolve_icon(icon_id: StringName) -> Resource` 方法返回占位资源
- 扩展系统落地后：切换为 `AssetRegistry.resolve(icon_id)` 调用
- 面板代码中对 `_resolve_icon` 的调用点即为未来扩展接入点

**ADR-UI-006：面板文本通过本地化 key 查询**
- 面板中所有面向用户的文本（标签、提示、结果文案）使用本地化 key
- 当前阶段：`LocalizationRegistry` 尚未落地，`_resolve_text(key: StringName) -> String` 直接返回 key 对应的硬编码中文
- 扩展系统落地后：切换为 `LocalizationRegistry.resolve(key)` 调用

### 1.4 扩展系统对接总览

UI 框架层需要对接扩展系统总体规划（[38-扩展系统总体规划-v1](../wiki/04-roadmap-reference/38-扩展系统总体规划-v1.md)）中定义的 4 类扩展包：

| 扩展包类型 | 对 UI 框架的影响 | 当前状态 | 对接接口 |
|-----------|----------------|---------|---------|
| `ui_theme_pack` | 驱动面板样式（颜色、字体、布局参数） | 规划中 | `UIThemeProfile` Resource |
| `asset_pack` | 提供图标、贴图等素材资源 | 规划中 | `_resolve_icon()` / `_resolve_asset()` |
| `content_pack` | 动态扩展卡池、棋盘尺寸、波次定义 | 已有最小入口 | CardBar/BoardOverlay 的动态刷新 |
| `localization_pack` | 驱动面板文本的多语言切换 | 规划中 | `_resolve_text()` |

**对接策略**：当前阶段不实现扩展系统本身，但在面板基类和具体面板中预留统一的方法签名（见 ADR-UI-004/005/006）。这些方法当前返回内联默认值，未来由扩展系统注册表替换实现。详见第 11 节。

---

## 2. CanvasLayer 分层策略

视口 960x540。

| Layer | 名称 | 用途 | 典型内容 |
|-------|------|------|----------|
| -1 | BackgroundLayer | 战斗背景 | 棋盘背景渲染（未来） |
| 0 | （默认世界空间） | 游戏对象 | BattleManager、实体、BoardOverlay、收集物 |
| 10 | DebugLayer | 调试信息 | DebugOverlay（已有） |
| 50 | HUDLayer | 战斗 HUD | BattleHUD 容器 |
| 80 | PopupLayer | 弹出层 | 暂停菜单、确认对话框（未来） |
| 100 | ScreenLayer | 全屏覆盖 | PhaseScreen（胜利/失败） |

### 场景节点树结构

```
BattleScene (Node2D)
├── BattleManager (Node2D)               -- 不含任何 UI 代码
│   ├── RuntimeEntities/
│   ├── BattleBoardState
│   ├── BattleCardState
│   ├── BattleEconomyState
│   ├── BattleFlowState
│   └── WaveRunner
├── BoardOverlay (Node2D, 世界空间)       -- 棋盘格渲染
├── CanvasLayer(layer=50) "HUDLayer"
│   └── BattleHUD (Control, FULL_RECT, MOUSE_FILTER_IGNORE)
│       ├── SunCounter
│       ├── CardBar
│       ├── WaveProgress
│       └── (未来: ShovelButton, PauseButton)
├── CanvasLayer(layer=100) "ScreenLayer"
│   └── PhaseScreen (CanvasLayer)
└── InputRouter (Node)                   -- 场景级输入管理
```

**BoardOverlay 定位**：世界空间 Node2D，与 BattleManager、实体处于同一坐标系。原因：
- `BattleBoardState.get_slot_world_position()` 返回世界坐标，BoardOverlay 直接消费，零转换开销
- BoardCellVisual 用 Area2D 碰撞检测，与世界空间实体天然匹配
- 与此前原型 `board_visual.gd` 的世界空间模式一致
- 未来 Camera 移动/缩放时，世界空间节点自动跟随

BattleHUD 根节点使用 `MOUSE_FILTER_IGNORE`，点击事件穿透到世界空间，仅子控件拦截。

---

## 3. 核心抽象

### 3.1 UIPanelBase — HUD 面板基类

```gdscript
# scripts/ui/ui_panel_base.gd
class_name UIPanelBase
extends Control

var _battle: Node = null
var _subscriptions: Array[Dictionary] = []
var _theme_profile: UIThemeProfile = null

func panel_setup(battle: Node, scenario: Resource, theme: UIThemeProfile = null) -> void:
    _battle = battle
    _theme_profile = theme if theme else UIThemeProfile.default()
    # 子类重写，订阅事件

func panel_teardown() -> void:
    for sub in _subscriptions:
        EventBus.unsubscribe(sub.event_name, sub.callback)
    _subscriptions.clear()

func _track_subscribe(event_name: StringName, callback: Callable) -> void:
    EventBus.subscribe(event_name, callback)
    _subscriptions.append({"event_name": event_name, "callback": callback})

func _resolve_icon(icon_id: StringName) -> Resource:
    # 当前：返回占位资源。未来：AssetRegistry.resolve(icon_id)
    return _default_icon(icon_id)

func _resolve_text(key: StringName) -> String:
    # 当前：返回硬编码文本。未来：LocalizationRegistry.resolve(key)
    return _default_text(key)

func _get_theme() -> UIThemeProfile:
    return _theme_profile

func _default_icon(_icon_id: StringName) -> Resource:
    return null

func _default_text(key: StringName) -> String:
    return str(key)
```

职责：
- 统一的 setup/teardown 生命周期
- 跟踪 EventBus 订阅，teardown 时自动清理
- 提供 `_battle` 引用用于状态查询
- 提供扩展预留接口：`_resolve_icon()`、`_resolve_text()`、`_get_theme()`
- 当前阶段这些方法返回内联默认值；待扩展系统落地后，子类可选择覆写或由框架层统一替换

### 3.2 UIScreenBase — 全屏覆盖基类

```gdscript
# scripts/ui/ui_screen_base.gd
class_name UIScreenBase
extends CanvasLayer

var _battle: Node = null
var _theme_profile: UIThemeProfile = null

func screen_setup(battle: Node, theme: UIThemeProfile = null) -> void:
    _battle = battle
    _theme_profile = theme if theme else UIThemeProfile.default()

func screen_teardown() -> void:
    pass

func _resolve_text(key: StringName) -> String:
    # 当前：返回硬编码文本。未来：LocalizationRegistry.resolve(key)
    return str(key)

func _get_theme() -> UIThemeProfile:
    return _theme_profile
```

现有 `BattleResultOverlay` 已遵循此模式（extends CanvasLayer, layer=100）。新增的 `_resolve_text()` 和 `_get_theme()` 与 UIPanelBase 保持一致。

### 3.3 BattleHUD — HUD 容器与生命周期管理

```gdscript
# scripts/ui/battle_hud.gd
class_name BattleHUD
extends Control

var _theme_profile: UIThemeProfile = null

func setup(battle: Node, scenario: Resource, theme: UIThemeProfile = null) -> void:
    _theme_profile = theme if theme else UIThemeProfile.default()
    for child in get_children():
        if child.has_method("panel_setup"):
            child.panel_setup(battle, scenario, _theme_profile)

func teardown() -> void:
    for child in get_children():
        if child.has_method("panel_teardown"):
            child.panel_teardown()
```

不使用参考项目的 MainGameSubManager 模式。原因：
- Open PVZ 已有 BattleSubsystemHost 作为协调机制
- 显式 setup/teardown 调用比 owner 引用更透明、更可测试
- UI 不是战斗子系统，不应混入子系统体系

### 3.4 UIThemeProfile — 主题资源定义

```gdscript
# scripts/ui/theme/ui_theme_profile.gd
class_name UIThemeProfile
extends Resource

@export var theme_id: StringName = &"default"

@export_group("SunCounter")
@export var sun_icon_id: StringName = &"sun"
@export var sun_counter_bg_color: Color = Color(0.2, 0.15, 0.05)
@export var sun_counter_text_color: Color = Color(1, 1, 0)
@export var sun_counter_font_size: int = 24

@export_group("CardBar")
@export var card_bg_color: Color = Color(0.25, 0.2, 0.1)
@export var card_selected_color: Color = Color(0.4, 0.6, 0.3)
@export var card_affordable_text_color: Color = Color(1, 1, 1)
@export var card_unaffordable_text_color: Color = Color(0.5, 0.5, 0.5)
@export var card_cooldown_overlay_color: Color = Color(0, 0, 0, 0.5)
@export var card_font_size: int = 16

@export_group("BoardOverlay")
@export var cell_normal_color: Color = Color(0.3, 0.5, 0.2, 0.3)
@export var cell_hover_color: Color = Color(0.4, 0.7, 0.3, 0.5)
@export var cell_occupied_color: Color = Color(0.5, 0.3, 0.2, 0.3)
@export var cell_invalid_color: Color = Color(0.7, 0.2, 0.2, 0.3)

@export_group("PhaseScreen")
@export var victory_text_color: Color = Color(0.2, 0.8, 0.2)
@export var defeat_text_color: Color = Color(0.9, 0.2, 0.2)
@export var overlay_color: Color = Color(0, 0, 0, 0.6)

@export_group("WaveProgress")
@export var wave_font_size: int = 18
@export var wave_normal_color: Color = Color(1, 1, 1)
@export var wave_final_color: Color = Color(1, 0.3, 0.3)

static func default() -> UIThemeProfile:
    return load("res://scripts/ui/theme/default_theme.tres") as UIThemeProfile
```

职责：
- 集中定义所有面板的视觉参数，面板通过 `_get_theme()` 读取
- 主仓内置 `default_theme.tres`，与代码中的内联默认值完全一致
- `ui_theme_pack` 可提供替代 `.tres`，由 BattleHUD 在 `setup()` 时注入
- 面板代码中不再出现 `Color(...)` 等视觉常量，全部从 `_get_theme()` 读取

**当前阶段**：定义 Resource 签名和默认 `.tres`，面板逐步迁移到读取 `_get_theme()`。不要求所有面板立即完成迁移，但新代码必须使用 `_get_theme()` 而非硬编码颜色。

---

## 4. 状态绑定模式

### 4.1 "拉取初值 + 订阅增量"模式

面板创建时需要当前状态（不仅是未来事件）：

```gdscript
func panel_setup(battle: Node, scenario: Resource) -> void:
    super.panel_setup(battle, scenario)
    _current_sun = int(battle.get_current_sun())        # 拉取初值
    _track_subscribe(&"resource.changed", _on_resource_changed)  # 订阅增量
```

### 4.2 事件 → 面板 → 叶子控件 流程

```
EventBus: resource.changed {after: 75, delta: 25}
  │
  ▼ SunCounter._on_resource_changed(event_data)
  │  读取 event_data.core["after"]
  │  更新 _label.text
  │  触发 scale tween 动画
  ▼
（叶控件更新完毕，无进一步传播）
```

含多子控件面板（CardBar）：

```
EventBus: resource.changed {after: 75}
  │
  ▼ CardBar._on_resource_changed(event_data)
  │  _current_sun = event_data.core.after
  │  _refresh_affordability()
  │  │  遍历 _card_slots，更新费用标签颜色
  │  ▼
（叶控件通过中介者直接操作更新）
```

### 4.3 可订阅的战斗事件清单

| 事件名 | 来源子系统 | 消费面板 |
|--------|-----------|---------|
| `resource.changed` | BattleEconomyState | SunCounter, CardBar |
| `sun.spawned` | BattleEconomyState | SunCounter（未来动画） |
| `sun.collected` | BattleEconomyState | SunCounter（飞入动画） |
| `card.cooldown_started` | BattleCardState | CardBar |
| `card.play_rejected` | BattleCardState | CardBar, InputRouter |
| `card.play_requested` | BattleCardState | CardBar |
| `placement.accepted` | BattleBoardState | BoardOverlay |
| `placement.rejected` | BattleBoardState | BoardOverlay |
| `entity.died` | HealthComponent | BoardOverlay |
| `wave.started` | WaveRunner | WaveProgress |
| `wave.completed` | WaveRunner | WaveProgress |
| `battle.phase_changed` | BattleFlowState | PhaseScreen |
| `battle.victory` | BattleFlowState | PhaseScreen |
| `battle.defeat` | BattleFlowState | PhaseScreen |

---

## 5. 输入桥接

### 5.1 UI → InputRouter（用户操作翻译为语义动作）

```
CardBar.card_selected(card_id)       → InputRouter.request_card_select(card_id)
BoardOverlay.cell_clicked(lane, slot) → InputRouter.request_cell_click(lane, slot)
SunCollectibleVisual.clicked         → BattleEconomyState.collect_sun()（直接调用）
ESC 键 (_unhandled_input)            → InputRouter.request_cancel()
```

### 5.2 InputRouter → UI（反馈信号，UI 内部关注）

```
InputRouter.card_selected(card_id)   → CardBar 高亮选中卡片
InputRouter.card_deselected          → CardBar 清除高亮
InputRouter.cell_hovered(lane, slot) → BoardOverlay 高亮格子
InputRouter.cell_unhovered(lane,slot) → BoardOverlay 清除高亮
```

反馈信号使用**直接信号**（非 EventBus），纯 UI 内部关注点。

### 5.3 键盘快捷键

通过 InputRouter 统一管理：
- `1-9`：选择对应位置卡片
- `S`：铲子模式（未来）
- `ESC`：取消选择
- `R`：重启战斗（场景级处理，不经过 InputRouter）

---

## 6. 组件规格

### 6.1 CardBar（已迁入 `scripts/ui/panels/card_bar.gd`）

| 项目 | 说明 |
|------|------|
| 基类 | UIPanelBase |
| 消费事件 | `resource.changed`, `card.cooldown_started`, `card.play_rejected`, `card.play_requested` |
| 发射信号 | `card_selected(card_id)`, `card_deselected` |
| 状态查询 | `BattleCardState.hand_order`, `_card_defs`, `_cooldown_ready_times` |
| 子控件 | PanelContainer × N（卡片槽），每槽含 ColorRect + Label(名称) + Label(费用) + ColorRect(冷却覆盖) |
| 迁移要点 | setup() → panel_setup()；直接订阅 → _track_subscribe()；新增 _battle 引用 |
| 扩展预留 | 颜色全部从 `_get_theme()` 读取；卡片图标使用 `_resolve_icon(card.icon_id)`；卡片名称使用 `_resolve_text(card.name_key)` |

补充参考：

- 原版 `de-pvz` 的 `SeedPacket / SeedBank` 值得借的不是界面样式，而是"卡槽状态是正式模型"这件事。
- 它把激活、冷却刷新、可拾取、传送带、老虎机等都建成了正式状态字段和更新逻辑。
- 对 Open PVZ 的启发是：后续 `CardBar` 不应长期停留在"若干按钮 + 局部动画"的层级，而应逐步形成明确的 packet state model，至少覆盖：
  - 可选中 / 不可选中
  - 冷却中
  - 费用不足
  - 特殊来源（未来如传送带或临时卡）

**扩展系统对接**：
- `content_pack` 注册新 `CardDef` 后，CardBar 应能响应 `content.registered` 事件动态刷新卡池（Phase 9 预留）
- 卡片来源标识：区分主仓卡片 vs 扩展包卡片（`card.pack_id` 字段，待 `ContentRegistry` 落地）

### 6.2 SunCounter（已迁入 `scripts/ui/panels/sun_counter.gd`）

| 项目 | 说明 |
|------|------|
| 基类 | UIPanelBase |
| 消费事件 | `resource.changed` |
| 子控件 | HBoxContainer（图标 ColorRect + Label） |
| 动画 | 数值变化时 scale tween |
| 扩展预留 | 图标使用 `_resolve_icon(_get_theme().sun_icon_id)`；背景色/文字色从 `_get_theme()` 读取 |

**扩展系统对接**：
- `asset_pack` 提供 `sun_icon_id` 对应的真实贴图后，SunCounter 自动切换（通过 `_resolve_icon()`）
- `ui_theme_pack` 可替换计数器配色方案

### 6.3 BoardOverlay（已迁入 `scripts/ui/panels/board_overlay.gd`）

| 项目 | 说明 |
|------|------|
| 基类 | Node2D（世界空间，不在 CanvasLayer 内） |
| 消费事件 | `placement.accepted`, `placement.rejected`, `entity.died` |
| 发射信号 | `cell_clicked(lane_id, slot_index)` |
| 子控件 | BoardCellVisual × (lane × slot)，Area2D 碰撞检测 |
| 状态 | normal / hover_valid / occupied / invalid |
| 迁移要点 | 保留 setup() 接口（世界空间面板不走 panel_setup），增加 _track_subscribe() |
| 扩展预留 | 格子状态色从 `_get_theme()` 读取（BoardOverlay 需单独持有 UIThemeProfile 引用） |

**扩展系统对接**：
- `content_pack` 中的 `BattlefieldPreset` 可能定义不同棋盘尺寸（lane 数 × slot 数），BoardOverlay 需根据 scenario 动态生成格子，而非硬编码 5×9
- `ui_theme_pack` 可替换格子状态配色

### 6.4 WaveProgress（已迁入 `scripts/ui/panels/wave_progress.gd`）

| 项目 | 说明 |
|------|------|
| 基类 | UIPanelBase |
| 消费事件 | `wave.started`, `wave.completed` |
| 子控件 | Label（文本），未来增加 ProgressBar |
| 特殊行为 | 最终波次变色 + scale 脉冲 |
| 扩展预留 | 颜色/字号从 `_get_theme()` 读取；文本通过 `_resolve_text()` 获取 |

### 6.5 PhaseScreen（已迁入 `scripts/ui/screens/phase_screen.gd`）

| 项目 | 说明 |
|------|------|
| 基类 | UIScreenBase |
| 消费事件 | `battle.victory`, `battle.defeat`, `battle.phase_changed` |
| 子控件 | ColorRect(半透明覆盖) + Label(结果文字) |
| 扩展 | 增加 preparing 阶段显示（未来） |
| 扩展预留 | 覆盖色/文字色从 `_get_theme()` 读取；结果文案通过 `_resolve_text()` 获取 |

---

## 7. 生命周期管理

### 7.1 战斗会话生命周期

```
1. 场景加载
2. 创建 BattleManager，加入场景树
3. BattleManager._ready() → reset_battle() → 子系统创建
4. 创建 UI 组件：
   a. 创建 CanvasLayer（HUDLayer, ScreenLayer）
   b. 创建 BattleHUD，添加面板（SunCounter, CardBar, WaveProgress）
   c. 创建 BoardOverlay（世界空间）
   d. 创建 PhaseScreen（ScreenLayer）
   e. 创建 InputRouter
   f. 调用 panel_setup(battle, scenario) / screen_setup(battle)
   g. 连接 UI 信号到 InputRouter
5. 游戏循环运行，UI 观察事件并更新
6. 战斗结束：
   a. PhaseScreen 显示结果
   b. InputRouter 进入 DISABLED 模式
   c. 玩家按 R → 重启
7. 场景退出：
   a. panel_teardown()（取消 EventBus 订阅）
   b. InputRouter 清理
```

### 7.2 重启流程

```
1. panel_teardown()  — 取消所有 EventBus 订阅
2. BattleManager.reset_battle()  — 清除 EventBus，重建子系统
3. panel_setup(battle, new_scenario)  — 重新订阅，拉取初值
```

**关键顺序**：先 teardown UI，再 reset BattleManager（因为 reset 会调用 EventBus.clear()）。

### 7.3 内存安全

- 所有 UI 节点是场景根的子节点，场景切换时自动释放
- Tween 动画在 teardown 中需要 kill，防止悬挂回调
- EventBus 订阅必须在 teardown 中显式取消

---

## 8. 文件组织

```
scripts/
├── ui/                              # 新增：UI 框架层
│   ├── ui_panel_base.gd             # HUD 面板基类 (Control)
│   ├── ui_screen_base.gd            # 全屏覆盖基类 (CanvasLayer)
│   ├── battle_hud.gd                # HUD 容器与生命周期
│   ├── panels/                      # 具体面板
│   │   ├── card_bar.gd              # 卡片选择栏
│   │   ├── sun_counter.gd           # 阳光计数器
│   │   ├── wave_progress.gd         # 波次进度
│   │   └── board_overlay.gd         # 棋盘渲染（世界空间）
│   ├── screens/                     # 全屏覆盖
│   │   └── phase_screen.gd          # 阶段过渡屏
│   └── theme/                       # 新增：主题资源
│       ├── ui_theme_profile.gd      # 主题 Resource 定义
│       └── default_theme.tres       # 主仓内置默认主题
├── input/                           # 新增：输入交互层
│   └── input_router.gd
├── validation/                      # 新增：验证脚本层
│   ├── card_place_validation.gd
│   └── sun_click_validation.gd
├── demo/                            # 场景编排与少量复用节点
│   ├── demo_battle_scene.gd
│   └── board_cell_visual.gd
```

**迁移策略**：第一批旧 demo UI / 输入脚本已完成替换并清理。后续若仍需保留原型代码，应只保留仍被新层复用或确有历史价值的最小节点。

---

## 9. 实施阶段

### Phase 0：基础骨架（无视觉变化）

创建文件和基类：
- `scripts/ui/ui_panel_base.gd`
- `scripts/ui/ui_screen_base.gd`
- `scripts/ui/battle_hud.gd`

**验证**：新文件可实例化无报错，现有验证场景全部通过。

### Phase 1：迁移 SunCounter（最简单面板，已完成）

- `scripts/ui/panels/sun_counter.gd` 继承 UIPanelBase
- 从 demo/sun_counter.gd 迁移逻辑
- 通过 BattleHUD 接线

**验证**：Demo 场景用新 SunCounter 表现一致。

### Phase 2：迁移 WaveProgress（已完成）

- `scripts/ui/panels/wave_progress.gd`
- 从 demo/wave_indicator.gd 迁移
- 增加进度条（total/complete）

**验证**：波次显示正确。

### Phase 3：迁移 CardBar（最复杂面板，已完成）

- `scripts/ui/panels/card_bar.gd`
- 多子控件、多事件订阅、信号发射
- 费用可购判断、冷却覆盖动画

**验证**：卡片选择、放置、冷却、可购性表现一致。

### Phase 4：迁移 BoardOverlay（已完成）

- `scripts/ui/panels/board_overlay.gd`
- 世界空间渲染，分离背景与交互
- BoardCellVisual 可复用

**验证**：格网渲染、hover/occupied/invalid 状态正确。

### Phase 5：迁移 PhaseScreen（已完成）

- `scripts/ui/screens/phase_screen.gd`
- 从 demo/battle_result_overlay.gd 迁移
- 增加 preparing 阶段显示

**验证**：胜利/失败画面正确显示。

### Phase 6：BattleHUD 编排集成（已完成）

- 创建使用新框架的 DemoBattleScene 变体
- 所有面板通过 BattleHUD 统一管理

**验证**：新场景与现有 demo 功能等价。

### Phase 7：InputRouter 集成（已完成）

- 替换 InputBridge 为 InputRouter
- CardBar/BoardOverlay 信号接入 InputRouter
- 键盘快捷键支持

**验证**：键盘操作（1-9, ESC）和鼠标操作均正常，现有验证场景不受影响。

### Phase 8：验证兼容性检查

- 运行全部 52 个验证场景
- 创建 `ui_framework_validation` 专用验证场景
- 确认 UI 可完全禁用

**验证**：全部场景通过，新场景验证 UI 生命周期。

### Phase 9：扩展系统接口预留

目标：在不依赖扩展系统实现的前提下，确保所有面板已为扩展系统对接做好准备。

具体工作：

1. **创建 `UIThemeProfile` Resource 和默认主题**
   - `scripts/ui/theme/ui_theme_profile.gd` — Resource 定义
   - `scripts/ui/theme/default_theme.tres` — 默认主题资源
   - 默认值与当前面板内联硬编码值完全一致

2. **面板迁移到 `_get_theme()` 读取视觉参数**
   - SunCounter：背景色、文字色、字号 → `_get_theme()`
   - CardBar：卡片背景色、选中色、可购/不可购文字色、冷却覆盖色 → `_get_theme()`
   - BoardOverlay：格子状态色 → `_get_theme()`（需单独持有 UIThemeProfile 引用）
   - WaveProgress：字号、常规色、最终波次色 → `_get_theme()`
   - PhaseScreen：覆盖色、胜/败文字色 → `_get_theme()`

3. **面板迁移到 `_resolve_icon()` 获取素材**
   - SunCounter：图标 ColorRect → `_resolve_icon(&"sun")` 占位（当前仍返回 ColorRect）
   - CardBar：卡片图标 → `_resolve_icon(card.icon_id)` 占位

4. **面板迁移到 `_resolve_text()` 获取文本**
   - 所有 Label.text 赋值点改为通过 `_resolve_text(key)` 获取
   - 当前 key 直接映射到硬编码中文，不做实际本地化

5. **BattleHUD 传递 theme 参数**
   - `setup(battle, scenario, theme)` 签名已更新
   - DemoBattleScene 创建 BattleHUD 时传入 theme（默认为 null，使用 default）

**验收标准**：
- 所有面板中无硬编码 `Color(...)` 调用（全部收敛到 `_get_theme()`）
- 所有面板中无直接文件路径引用素材（全部通过 `_resolve_icon()`）
- 所有面向用户的文本通过 `_resolve_text()` 获取
- 替换 `default_theme.tres` 中的颜色值后，所有面板视觉跟随变化
- 现有 52+ 验证场景全部通过
- 新增 `ui_theme_switch_validation` 验证场景：运行时替换 UIThemeProfile 后面板视觉正确更新

---

## 10. 验证兼容性保障

### 10.1 现有场景为何安全

当前 52 个验证场景均使用编程驱动模式：
- 创建 BattleManager，加载 BattleScenario .tres
- 通过 `card_play_requests` 和 `resource_spend_requests` 驱动
- **不创建任何 UI 组件**
- 不依赖 InputBridge 或任何输入处理

### 10.2 保护措施

1. **纯增量添加** — `scripts/ui/` 不修改 `scripts/battle/`、`autoload/`、`scripts/core/` 中任何现有文件
2. **BattleManager 不变** — 唯一连接点是 `panel_setup(battle)` 参数
3. **订阅清理** — teardown 先于 EventBus.clear()
4. **验证关卡** — Phase 8 显式运行全部场景 + 创建 UI 专用场景

---

## 11. 扩展系统集成策略

本节描述 UI 框架层与扩展系统总体规划（[38-扩展系统总体规划-v1](../wiki/04-roadmap-reference/38-扩展系统总体规划-v1.md)）的集成路线。

### 11.1 对接矩阵

| 扩展包类型 | 影响的面板 | 对接接口 | 预计落地阶段 |
|-----------|-----------|---------|-------------|
| `ui_theme_pack` | 所有面板 | `UIThemeProfile` Resource 注入 | 扩展总规第三步（升级内容扩展系统） |
| `asset_pack` | SunCounter, CardBar | `_resolve_icon()` → `AssetRegistry.resolve()` | 扩展总规第二步（素材包系统） |
| `content_pack` | CardBar, BoardOverlay | 动态卡池刷新、动态棋盘尺寸 | 扩展总规第三步（升级内容扩展系统） |
| `localization_pack` | 所有面板 | `_resolve_text()` → `LocalizationRegistry.resolve()` | 扩展总规后续（本地化包） |

### 11.2 各对接点的详细策略

#### 11.2.1 UI 主题包对接

当 `ui_theme_pack` 落地后：

1. `PackRegistry` 加载 `ui_theme_pack`，在 manifest 的 `entry_points` 中找到主题资源路径
2. `AssetRegistry` 或专用 `ThemeRegistry` 将主题 Resource 注册到全局
3. BattleHUD.setup() 改为从注册表查询当前激活主题：
   ```gdscript
   # 未来实现（非当前阶段）
   var theme := ThemeRegistry.get_active_theme() as UIThemeProfile
   setup(battle, scenario, theme)
   ```
4. 集合包（`collection_pack`）可在 manifest 中声明 `recommended_ui_theme`，BattleScene 读取后传入

#### 11.2.2 素材包对接

当 `AssetRegistry` 落地后：

1. 面板基类的 `_resolve_icon()` 切换实现：
   ```gdscript
   # 当前实现
   func _resolve_icon(icon_id: StringName) -> Resource:
       return _default_icon(icon_id)

   # 未来实现（切换为 AssetRegistry）
   func _resolve_icon(icon_id: StringName) -> Resource:
       return AssetRegistry.resolve(icon_id)
   ```
2. 面板代码中所有调用 `_resolve_icon()` 的点无需修改，只需修改基类实现
3. `AssetRegistry` 遵循回退链：当前激活素材包 → 依赖素材包 → 默认占位素材包

#### 11.2.3 内容包动态扩展对接

当 `ContentRegistry` 落地后：

1. CardBar 订阅 `content.registered` / `content.unregistered` 事件（新增），动态刷新卡槽
2. BoardOverlay 在 `panel_setup()` 时从 scenario 读取 `BattlefieldPreset` 的棋盘尺寸，动态生成格子
3. 卡片定义增加 `pack_id` 字段，CardBar 可在 UI 上区分来源（如显示扩展包标记）

#### 11.2.4 本地化包对接

当 `LocalizationRegistry` 落地后：

1. 面板基类的 `_resolve_text()` 切换实现：
   ```gdscript
   # 当前实现
   func _resolve_text(key: StringName) -> String:
       return _default_text(key)

   # 未来实现（切换为 LocalizationRegistry）
   func _resolve_text(key: StringName) -> String:
       return LocalizationRegistry.resolve(key)
   ```
2. 面板代码中所有 Label.text 赋值点已通过 `_resolve_text()` 获取，无需修改

### 11.3 不变量保障

UI 框架在与扩展系统集成时，以下不变量必须维持：

1. **UI 不驱动规则逻辑** — 即使 `ui_theme_pack` 替换了视觉，也不影响战斗计算
2. **UI 可选** — 扩展系统不依赖 UI 框架存在即可运作，无 UI 时战斗运行不受影响
3. **BattleManager 不变** — BattleManager 不引用任何 UI 类型，扩展系统的 UI 对接仅影响 UI 层
4. **验证场景兼容** — 所有验证场景继续不创建 UI，扩展系统接入不影响验证通过率

### 11.4 与扩展总规落地顺序的对齐

| 扩展总规阶段 | UI 框架层对应动作 |
|-------------|-----------------|
| 第一步：统一总图 | 当前（Phase 9）已完成接口预留 |
| 第二步：素材包系统 | `_resolve_icon()` 切换为 `AssetRegistry.resolve()` |
| 第三步：升级内容扩展系统 | CardBar/BoardOverlay 动态刷新；`UIThemeProfile` 从注册表加载 |
| 第四步：包级验证 | 新增 `ui_theme_pack` / `asset_pack` 的 UI 兼容性验证场景 |
| 第五步：包管理 UI | 新增扩展管理面板（不在本方案范围内，但基于 `UIPanelBase` 扩展） |

---

## 关键文件索引

| 文件 | 角色 |
|------|------|
| `scripts/demo/demo_battle_scene.gd` | 当前场景编排器与接线参考 |
| `scripts/ui/panels/card_bar.gd` | 现行卡片面板实现 |
| `scripts/ui/panels/sun_counter.gd` | 现行阳光计数器实现 |
| `scripts/ui/panels/board_overlay.gd` | 现行世界空间棋盘层实现 |
| `scripts/ui/panels/wave_progress.gd` | 现行波次面板实现 |
| `scripts/input/input_router.gd` | 现行输入路由器实现 |
| `scripts/ui/screens/phase_screen.gd` | 现行结果覆盖层实现 |
| `scripts/demo/board_cell_visual.gd` | 当前仍被 BoardOverlay 复用的格子可视节点 |
| `scripts/ui/theme/ui_theme_profile.gd` | 主题 Resource 定义（Phase 9 新增） |
| `scripts/ui/theme/default_theme.tres` | 主仓内置默认主题（Phase 9 新增） |
| `autoload/EventBus.gd` | 事件总线，UI 订阅/退订的契约 |
| `scripts/battle/battle_card_state.gd` | 卡片状态事件源 |
| `scripts/battle/battle_economy_state.gd` | 经济状态事件源 |
| `scripts/battle/battle_flow_state.gd` | 流程状态事件源 |
| `scripts/core/runtime/extension_pack_catalog.gd` | 扩展包注册表（未来 AssetRegistry/ContentRegistry 的前身） |
| `wiki/01-overview/34-Open PVZ 系统版图与规划分层.md` | 系统圈层定义 |
| `wiki/04-roadmap-reference/38-扩展系统总体规划-v1.md` | 扩展系统总体规划 |
| `wiki/04-roadmap-reference/39-素材包系统设计草案.md` | 素材包系统设计 |
| `wiki/04-roadmap-reference/41-扩展包-manifest-规范-v1.md` | 扩展包 manifest 规范 |
| `wiki/04-roadmap-reference/pvz-godot-dream/03-组件系统.md` | 参考项目组件架构 |
| `wiki/04-roadmap-reference/pvz-godot-dream/04-管理器与全局服务.md` | 参考项目管理器体系 |

---

## 相关文档

- [Open PVZ 系统版图与规划分层](../wiki/01-overview/34-Open%20PVZ%20系统版图与规划分层.md)
- [当前阶段与实现路线](../wiki/01-overview/23-当前阶段与实现路线.md)
- [开发路线图](../wiki/04-roadmap-reference/26-开发路线图.md)
- [扩展系统总体规划-v1](../wiki/04-roadmap-reference/38-扩展系统总体规划-v1.md)
- [素材包系统设计草案](../wiki/04-roadmap-reference/39-素材包系统设计草案.md)
- [扩展包 manifest 规范-v1](../wiki/04-roadmap-reference/41-扩展包-manifest-规范-v1.md)
- [参考实现-组件系统](../wiki/04-roadmap-reference/pvz-godot-dream/03-组件系统.md)
- [参考实现-管理器与全局服务](../wiki/04-roadmap-reference/pvz-godot-dream/04-管理器与全局服务.md)
