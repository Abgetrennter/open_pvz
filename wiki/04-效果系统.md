...
- `EffectDef.slots` 和参数约束要继续补成真正的协议校验层。
...
|------|------|
| `effect_id` | 效果唯一标识 |
| `slots` | 未来参数结构和子槽位说明的承载位 |
| `tags` | 分类或后续约束信息的承载位 |

当前要点：

- `EffectDef` 已经是正式 `Resource` 类型。
- 当前主干还没有把 `slots` 校验真正跑满。
- 所以 `EffectDef` 现在更像“协议骨架”，而不是已经成熟的完整 schema。
...
- 目标解析已经进入主干
- 但还没有被整理成最终版协议文档或资源 schema

所以它属于“当前事实”，但还不是“最终冻结协议”。
