...
| 正式 archetype 资源 | 正式内容与扩展层 | 已形成 v1 基线 | 当前共 `97` 个 archetype |
| 扩展包入口 | 正式内容与扩展层 | 已进入主干 | 最小包、chaos 包、guardrail 包均已纳入回归 |
| 验证矩阵与批量回归 | 展示、验证与治理层 | 已进入主干 | `140` 个场景，支持分层回归 |
...
当前旧实体模型只允许作为历史归档语义出现，不能再回到正文主叙事。
