...
```mermaid
graph TD
    A["Autoload<br/>EventBus / DebugService / SceneRegistry<br/>TriggerRegistry / EffectRegistry / GameState<br/>MechanicFamilyRegistry / MechanicTypeRegistry<br/>MechanicCompilerRegistry / ControllerRegistry<br/>DetectionRegistry / ProjectileMovementRegistry"] --> B["BattleManager"]
...
- **投射体是连续行为的入口之一**，Projectile movement 通过 `ProjectileMovementRegistry` 分发；Controller 是另一个入口（bite / sweep 等），通过 `_physics_process` 持续模拟
...
- `ProjectileMovementDef`、`MechanicCompilerDef`（通用扩展插槽贡献项）
- `HeightBand`、`MovementContributionDef`

### 编译与运行时 (`scripts/core/runtime/`)

- `MechanicCompiler`（49 个内置 type 的编译引擎）
