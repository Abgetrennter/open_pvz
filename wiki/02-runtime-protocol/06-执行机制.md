...
- battle mode 组织层通过 `BattleModeHost` 继续消费 battle / input 事件，并在 battle 主链之上追加 mode-level 规则与 objective 判定。
- 所有随机行为通过确定性随机协议（`battle_seed → entity_seed → mechanic_seed → ShuffleBag`）保证可复现。

### 当前仍未收口

- 更系统的执行链可视化仍未成为正式主干能力
- 协议变更与兼容策略仍主要由 ADR / 治理页维护，而不是由执行机制正文内建版本化协议
...
- [`scripts/battle/mode/battle_mode_host.gd`](../../scripts/battle/mode/battle_mode_host.gd)
