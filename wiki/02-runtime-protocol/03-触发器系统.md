...
|------|------|------|
| `periodically` | `game.tick` | `game_time - last_triggered_time >= interval`，并可选执行 `detection_id` 检测条件 |
...
| `trigger_id` | 正式条件字段 |
|------|------|
| `periodically` | `interval`、`detection_id`、`scan_range`、`start_delay` |
...
- 白名单外条件默认报错
- 条件类型错误不会再被默默兼容
- `periodically` 当前已支持最小索敌 gating，但仍保持“是否触发”这一层职责，不直接改写效果层
