...
- 当前主干已经稳定接入 `damage`、`spawn_projectile`、`explode`、`produce_sun` 四个内置效果。
...
graph TD
    ...
    D --> E["Damage / Spawn Projectile / Explode / Produce Sun"]
```
...
| `produce_sun` | 在实体位置生成阳光收集物 | `BattleEconomyState.spawn_sun()` |
...
- 通过 `periodically` 触发器驱动，实现周期性阳光生产
- 调用 `BattleEconomyState.spawn_sun()` 在实体位置上方生成阳光收集物
- 参数：`value`（阳光值）、`source_type`（来源标签）、`offset_y`（生成位置偏移）
- 不需要目标解析——阳光生成在实体自身位置
...
- [扩展系统总体规划](../04-roadmap-reference/38-扩展系统总体规划.md)（草案层：`plans/draft/extension-system/37-扩展包新增效果与效果外置策略.md`）
